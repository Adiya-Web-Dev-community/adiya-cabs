      ...
      <div className="bg-gray-100 rounded-lg overflow-hidden">
        <table className="border-[1px] w-full">
          <thead className="border-b bg-black text-white rounded-t-lg">
            <tr>
              <th className="px-1 py-2 italic text-md text-left">
                Booking Date
              </th>
              <th className="px-1 py-2 italic text-md text-left">
                Passenger Name
            ...
            {data.map((ele, i) => (
              ...
              >
                ...
                <td className="px-2 py-2 text-sm text-left">
                  {ele.passengerId.email && ele.passengerId.email.length > 25
                    ? `${ele.passengerId.email.slice(0, 25)}...`
                ...
                <td className="px-2 py-2 text-sm text-left">
                  {ele.pickupLocation && ele.pickupLocation.length > 35
                    ? `${ele.pickupLocation.slice(0, 35)}...`
                    : ele.pickupLocation || "---"}
                </td>
                <td className="px-2 py-2 text-sm text-left">
